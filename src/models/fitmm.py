...
class FITMM(GeneralRecommender):
    ...
    def compute_normalized_laplacian(self, indices, adj_size):
        adj_values = torch.ones(indices.shape[1], dtype=torch.float32, device=indices.device)
        adj = torch.sparse_coo_tensor(indices, adj_values, adj_size, device=self.device).coalesce()
        row_sum = 1e-7 + torch.sparse.sum(adj, -1).to_dense()
        r_inv_sqrt = torch.pow(row_sum, -0.5)
        rows_inv_sqrt = r_inv_sqrt[indices[0]]
        cols_inv_sqrt = r_inv_sqrt[indices[1]]
        values = rows_inv_sqrt * cols_inv_sqrt
        return torch.sparse_coo_tensor(indices, values, adj_size, device=self.device).coalesce()
...
class FrequencyDecompositionModule(nn.Module):
    def __init__(self, dim_latent, id_dim, v_dim, t_dim, M, ib_direction, ib_alpha, ib_mu, ib_phi_plus, hidden_dim=128):
        ...
        self._svd_nonfinite_warned = False
        self._svd_fallback_warned = False
    ...
    @staticmethod
    def _is_svd_error(error):
        message = str(error).lower()
        return "svd" in message or "linalg" in message or "cusolver" in message

    def _eigh_svd_fallback(self, rep):
        """Recover an SVD-like decomposition from the feature Gram matrix."""
        origin_device = rep.device
        origin_dtype = rep.dtype
        work = rep.to(device="cpu", dtype=torch.float64)
        n_features = work.shape[1]
        eps = torch.finfo(work.dtype).eps * max(1, n_features)

        gram = work.transpose(0, 1).matmul(work)
        gram = gram + eps * torch.eye(n_features, dtype=work.dtype, device=work.device)
        ...
        U = work.matmul(eigvecs) / S.clamp_min(eps).unsqueeze(0)
        Vh = eigvecs.transpose(0, 1)
        return (
            U.to(device=origin_device, dtype=origin_dtype),
            S.to(device=origin_device, dtype=origin_dtype),
            Vh.to(device=origin_device, dtype=origin_dtype),
        )

    def _safe_svd(self, rep):
        if not torch.isfinite(rep).all():
            self._warn_once(
                "_svd_nonfinite_warned",
                "FITMM SVD input contains NaN or Inf; replacing non-finite values with 0.0.",
            )
            rep = torch.nan_to_num(rep, nan=0.0, posinf=0.0, neginf=0.0)

        try:
            return torch.linalg.svd(rep, full_matrices=False)
        except Exception as error:
            if not self._is_svd_error(error):
                raise

        self._warn_once(
            "_svd_fallback_warned",
            "FITMM SVD failed to converge on the current device; falling back to CPU/eigh decomposition.",
        )
        origin_device = rep.device
        origin_dtype = rep.dtype
        try:
            U, S, Vh = torch.linalg.svd(rep.to(device="cpu", dtype=torch.float32), full_matrices=False)
            return (
                U.to(device=origin_device, dtype=origin_dtype),
                S.to(device=origin_device, dtype=origin_dtype),
                Vh.to(device=origin_device, dtype=origin_dtype),
            ...
            if not self._is_svd_error(error):
                raise
            return self._eigh_svd_fallback(rep)
